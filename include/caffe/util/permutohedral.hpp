...
/*! \brief Hash Table for keeping track of lattice occupancy. Taken from Krähenbühl's
 *         original DenseCRF lattice code.
...
/*! \brief Class functions for traversing the lattice to build neighborhood stucture
 *         for convolutions.
 */
...
class LatticeTraversal {
 ...
 public:
  explicit
  LatticeTraversal(int neighborhood_size, int d)
 ...
 private:
  template <typename TFun>
  void walk_cuboid(const key_type& start_key, const int d,
                   const bool has_zero, std::vector<key_type>& walking_keys,
                   TFun yield) const;
 ...
 *         traversing the lattice to build neighborhood stucture for convolutions.
 */

class LatticeApproximateTraversal {
 public:
  LatticeApproximateTraversal(
      int neighborhood_size, int d,
      const std::vector<int>& immediate_neighbors, int M)
    : neighborhood_size_(neighborhood_size), d_(d),
      immediate_neighbors_(immediate_neighbors), M_(M) {
  }
 ...
 private:
  template <typename TFun>
  void walk_approximate(const int start, const int d,
                        const bool has_zero, TFun yield) const;
...
class GaussianFilter {
 ...
 private:
  class TraversalCallback {
   public:
    TraversalCallback(HashTable& hash_table)
...
/*! \brief This is the main class for lattice construction and forward operations in
 *         'learnable' sparse high dimensional filtering.
 *         This class defines 'forward' functionatlity. See CPU/GPU specific
 *         'PermutohedralReverseCpu' and 'PermutohedralReverseGpu' for the definition of
 *         'splat', 'blur', 'max' and 'slice' forward functions and
 *         the respective backward functions.
 *
 *  'Filter' weights are generic non-seperable high-dimensional permutohedral filters.
 *  This class has both CPU and GPU functionality except for the lattice construction,
 *  At present, there is no dedicated GPU functions for permutohedral lattice construction.
 *
 *  Some parts of the code are adapted and heavily modified from the separable filter code from
 *  Adams et al. 2010 (http://graphics.stanford.edu/papers/permutohedral/).
 */
 ...
 *         'PermutohedralReverseCpu' and 'PermutohedralReverseGpu' for the definition of
 *         'splat_tick', 'blur_tick', 'max_tick' and 'slice_tick' backward functions and
 *         the respective foward functions.
 *
 */
...
class PermutohedralReverse {
 public:
  ...
  virtual
  void reverse(const value_type* diff_in, value_type* diff_out_filter,
               value_type* diff_out_in) = 0;

  virtual
  void max_reverse(const value_type* diff_in, value_type* diff_out_in) = 0;
};
...
class Permutohedral {
 ...
 private:
  ...
  static
  void map_back(const std::vector<boost::int16_t>& key, value_type* const x);
 ...
 public:
  ...
  void init(const value_type* feature, int data_count, int
            feature_size, int neighborhood_size, bool do_visualization);

  boost::shared_ptr<PermutohedralReverse<value_type> >
  compute(const value_type* filter, const value_type* in,
          int num_output, int group,
          int value_size, bool do_skip_blur,
          int in_offset, int out_offset,
          int in_size, int out_size,
          value_type* out) const;

  boost::shared_ptr<PermutohedralReverse<value_type> >
  max_compute(const value_type* filter, const value_type* in,
          int value_size,
          int in_offset, int out_offset,
          int in_size, int out_size,
          value_type* out);
  ...
  boost::shared_ptr<PermutohedralReverse<value_type> >
  compute_gpu(const value_type* filter, const value_type* in,
          int num_output, int group,
          int value_size, bool do_skip_blur,
          int in_offset, int out_offset,
          int in_size, int out_size,
  ...
  boost::shared_ptr<PermutohedralReverse<value_type> >
  max_compute_gpu(const value_type* filter, const value_type* in,
          int value_size,
          int in_offset, int out_offset,
          int in_size, int out_size,
          value_type* out);
...
class PermutohedralReverseCpu : public PermutohedralReverse<T> {
 ...
 private:
  ...
  void init(const value_type* filter,
            int num_output, int group, int value_size, bool do_skip_blur,
            int in_offset, int out_offset, int in_size, int out_size,
            const boost::shared_ptr<
            const typename Permutohedral<value_type>::Lattice> lattice);

  void compute(const value_type* in, value_type* out);

  void max_compute(const value_type* in, value_type* out);

  void slice(const std::vector<value_type>& data,
  ...
  static
  void im2col(const value_type* im,
              const std::size_t value_size,
              const std::size_t filter_size, const std::size_t M,
              const std::size_t start, const std::size_t end,
              const std::vector<int>& blur_neighbors,
              value_type* col);

  static
  void col2im(const value_type* col,
              const std::size_t value_size,
              const std::size_t filter_size, const std::size_t M,
              const std::size_t start, const std::size_t end,
              const std::vector<int>& blur_neighbors,
...
class PermutohedralReverseGpu : public PermutohedralReverse<T> {
 ...
 private:
  ...
  void slice(const Blob<value_type>& data,
             value_type* sliced) const;
  ...
  void max(const Blob<value_type>& splatted,
