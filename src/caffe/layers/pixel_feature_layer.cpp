    ...
    const vector<Blob<Dtype>*>& top)
{
    ...
    const PixelFeatureParameter& parameter = this->layer_param_.pixel_feature_param();
            ...
            for (unsigned int y = 0; y < height_; ++y) {
                ...
                for (unsigned int x = 0; x < width_; ++x) {
                    ...
                    for (unsigned int c = 0; c < bottom[0]->channels(); ++c) {
                        top_data[top[0]->offset(n, c + 2, y, x)] = color_scale * bottom[0]->data_at(n, c, y, x);
                    }
                }
            }
        }
    ...
    case PixelFeatureParameter_Feature_RGB_AND_POSITION: {
        ...
        for (unsigned int n = 0; n < num_; ++n) {
            for (unsigned int y = 0; y < height_; ++y) {
                ...
                for (unsigned int x = 0; x < width_; ++x) {
                    for (unsigned int c = 0; c < bottom[0]->channels(); ++c) {
                        top_data[top[0]->offset(n, c, y, x)] = color_scale * bottom[0]->data_at(n, c, y, x);
                    }
                    top_data[top[0]->offset(n, bottom[0]->channels(), y, x)] = y_offset;
                    top_data[top[0]->offset(n, bottom[0]->channels() + 1, y, x)] = scale * x + offset_w;
                }
            }
        }
        break;
            ...
            for (unsigned int n = 0; n < num_; ++n) {
                for (unsigned int y = 0; y < height_; ++y) {
                    ...
                    for (unsigned int x = 0; x < width_; ++x) {
                        ...
                        top_data[top[0]->offset(n, 0, y, x)] = sinAngle * scaled_x + cosAngle * scaled_y + mid_y;
                        top_data[top[0]->offset(n, 1, y, x)] = cosAngle * scaled_x - sinAngle * scaled_y + mid_x;
            ...
            for (unsigned int n = 0; n < num_; ++n) {
                for (unsigned int y = 0; y < height_; ++y) {
                    ...
                    for (unsigned int x = 0; x < width_; ++x) {
                        ...
                        top_data[top[0]->offset(n, 0, y, x)] = sinAngle * scaled_x + cosAngle * (scaled_y - mid_y) + mid_y;
                        top_data[top[0]->offset(n, 1, y, x)] = cosAngle * scaled_x - sinAngle * scaled_y + mid_x;
                    }
                }
            }
