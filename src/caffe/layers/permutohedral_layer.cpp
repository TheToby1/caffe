...
void PermutohedralLayerTemplate<Dtype, Ptype>::LayerSetUp(
  ...
  if (permutohedral_param.offset_type() ==
    ...
    CHECK_EQ(num_output_, channels_ / group_) <<
      "Diagonal offset can only be used when num_output = num_channels / group";
  }
...
void PermutohedralLayerTemplate<Dtype, Ptype>::InitLattice(
  ...
  if (!operations_.empty() && (!operations_[0].blur_
                               || do_repeated_init_)) {
  ...
  #pragma omp parallel
  {
    ...
    for (int n = 0; n < num_; ++n) {
      ...
      op.reverse_->reverse(scaled_back_data,
                           thread_filter_diff, bottom_diff +
                           bottom_blob.offset(n));
