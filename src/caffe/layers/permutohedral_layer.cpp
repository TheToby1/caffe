...
{
    PermutohedralParameter permutohedral_param = this->layer_param_.permutohedral_param();
    ...
    if (permutohedral_param.offset_type() == PermutohedralParameter_OffsetType_DIAG) {
        CHECK_EQ(num_output_, channels_ / group_) << "Diagonal offset can only be used when num_output = num_channels / group";
    }

    if (do_skip_blur_) {
        CHECK_EQ(channels_, num_output_) << "Number of output channels sould be same as input when skipping blur";
    }
    ...
    if (!this->blobs_.empty()) {
        ...
        if (bias_term_) {
            ...
            shared_ptr<Filler<Dtype>> bias_filler(GetFiller<Dtype>(
                permutohedral_param.bias_filler()));
        ...
        this->blobs_[0].reset(
            new Blob<Dtype>(
                ...
                permutohedral_type::get_filter_size(
                    neighborhood_size_, feature_size_)));
...
void PermutohedralLayerTemplate<Dtype, Ptype>::InitLattice(
    ...
    const Blob<Dtype>& in_feature_blob, const Blob<Dtype>& out_feature_blob,
    Blob<Dtype>* const lattice_blob)
{
    ...
    if (!operations_.empty() && (!operations_[0].blur_ || do_repeated_init_)) {
        ...
        typename permutohedral_type::gauss_type gauss(
            neighborhood_size_, feature_size_);
        const Dtype* gauss_filter = gauss.filter();

        Dtype* lattice_blob_data = 0;
        if (lattice_blob)
            lattice_blob_data = lattice_blob->mutable_cpu_data();
        ...
        // #pragma omp parallel
        {
            ...
            for (int n = 0; n < num_; ++n) {
                for (int c = 0; c < feature_size_; ++c) {
                    caffe_copy(in_size,
                        in_feature_data + in_feature_blob.offset(n, c),
                        feature.data() + c * data_count);
                    caffe_copy(out_size,
                        out_feature_data + out_feature_blob.offset(n, c),
                ...
                if (!in_lattice_blob_data) {
                    ...
                    op.blur_->init(feature.data(),
                        data_count, feature_size_,
                        neighborhood_size_, do_visualization_);

                    if (!op.norm_there_)
                        op.norm_there_ = boost::make_shared<Blob<Dtype>>(
                            1, 1, in_height_, in_width_);
                    if (!op.norm_back_)
                        op.norm_back_ = boost::make_shared<Blob<Dtype>>(
                            1, 1, out_height_, out_width_);
                    ...
                    case PermutohedralParameter_NormType_AFTER:
                        op.blur_->compute(gauss_filter, norm_there_data,
                            1, 1, 1, false, in_offset, out_offset, in_size, out_size,
                            norm_back_data);
                    ...
                    case PermutohedralParameter_NormType_SYMMETRIC:
                        op.blur_->compute(gauss_filter, norm_there_data,
                            1, 1, 1, false, in_offset, in_offset, in_size, in_size,
                            norm_there_data);

                        std::fill(norm_back_data, norm_back_data + out_size, 1);
                        op.blur_->compute(gauss_filter, norm_back_data,
                            1, 1, 1, false,
                        ...
                        for (int i = 0; i < op.norm_there_->count(); ++i) {
                            norm_there_data[i] = 1.0 / (std::sqrt(norm_there_data[i] + 1e-20));
                        }

                        for (int i = 0; i < op.norm_back_->count(); ++i) {
                            norm_back_data[i] = 1.0 / (std::sqrt(norm_back_data[i] + 1e-20));
                        }
                        break;
                    }
                    ...
                    if (lattice_blob_data) {
                        *reinterpret_cast<boost::shared_ptr<permutohedral_type>**>(
                            lattice_blob_data + lattice_blob->offset(n, 0))
                            = &op.blur_;
                        *reinterpret_cast<boost::shared_ptr<Blob<Dtype>>**>(
                            lattice_blob_data + lattice_blob->offset(n, 1))
                            = &op.norm_there_;
                        *reinterpret_cast<boost::shared_ptr<Blob<Dtype>>**>(
                            lattice_blob_data + lattice_blob->offset(n, 2))
                            = &op.norm_back_;
                    }
                } else {
                    ...
                    op.norm_there_ = **reinterpret_cast<
                        const boost::shared_ptr<Blob<Dtype>>* const*>(
                        in_lattice_blob_data + in_lattice_blob->offset(n, 1));
                    op.norm_back_ = **reinterpret_cast<
                        const boost::shared_ptr<Blob<Dtype>>* const*>(
    ...
    const Blob<Dtype>& filter_blob, Blob<Dtype>* shifted_filter_blob) const
{
    ...
    const int filter_size = permutohedral_type::get_filter_size(
        neighborhood_size_, feature_size_);
    shifted_filter_blob->Reshape(num_output_, channels_ / group_, 1,
        filter_size);
        ...
        for (int n = 0; n < num_output_; ++n) {
            for (int c = 0; c < channels_ / group_; ++c) {
                for (int i = 0; i < filter_size; ++i) {
                    shifted_filter[shifted_filter_blob->offset(n, c, 0, i)] = filter[filter_blob.offset(n, c, 0, i)] + gauss_filter[i];
                }
            }
        }
        break;
    case PermutohedralParameter_OffsetType_DIAG:
        std::copy(filter, filter + shifted_filter_blob->count(),
            shifted_filter);
        for (int n = 0; n < num_output_; ++n) {
            for (int i = 0; i < filter_size; ++i) {
                shifted_filter[shifted_filter_blob->offset(n, n, 0, i)] += gauss_filter[i];
            }
        }
        break;
    ...
    // #pragma omp parallel
    {
        ...
        for (int n = 0; n < num_; ++n) {
            ...
            for (int c = 0; c < num_output_; ++c) {
                caffe_mul(out_size,
                    top_data + top_blob.offset(n, c), norm_back_data,
            ...
            if (bias_term_) {
                caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_,
                    out_data_count, 1, (Dtype)1.,
                    this->blobs_[1]->cpu_data(),
                    bias_multiplier_.cpu_data(),
                    (Dtype)1., top_data + top_blob.offset(n));
            }
        }
    }
}
    ...
    const vector<Blob<Dtype>*>& bottom)
{
    ...
    if (bias_term_) {
        ...
        for (int n = 0; n < num_; ++n) {
            caffe_cpu_gemv<Dtype>(CblasNoTrans, num_output_, out_data_count,
                1., top_diff + top_blob.offset(n),
    ...
    // #pragma omp parallel
    {
        ...
        Blob<Dtype> thread_filter_blob(filter_blob.num(),
            filter_blob.channels(),
            filter_blob.height(),
            filter_blob.width());
        ...
        for (int n = 0; n < num_; ++n) {
            ...
            op.reverse_->reverse(scaled_back_data,
                thread_filter_diff, bottom_diff + bottom_blob.offset(n));

            for (int c = 0; c < channels_; ++c) {
                caffe_mul(in_size,
                    bottom_diff + bottom_blob.offset(n, c), norm_there_data,
                    bottom_diff + bottom_blob.offset(n, c));
            }
        }
...
template class PermutohedralLayerTemplate<
    float, permutohedral::Permutohedral>;
template class PermutohedralLayerTemplate<
    double, permutohedral::Permutohedral>;
template class PermutohedralLayerTemplate<
    float, permutohedral::GaussPermutohedral>;
template class PermutohedralLayerTemplate<
    double, permutohedral::GaussPermutohedral>;

template <typename Dtype>
shared_ptr<Layer<Dtype>>
Creator_PermutohedralLayer(const LayerParameter& param)
{
    return shared_ptr<Layer<Dtype>>(
        new PermutohedralLayerTemplate<Dtype, permutohedral::Permutohedral>(
            param));
}

template <typename Dtype>
shared_ptr<Layer<Dtype>>
Creator_GaussPermutohedralLayer(const LayerParameter& param)
{
    return shared_ptr<Layer<Dtype>>(
