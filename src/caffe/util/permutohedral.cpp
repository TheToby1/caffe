...
template <typename TFun>
void LatticeTraversal::walk_cuboid(
    const key_type& start_key, const int d,
  ...
  if (d <= d_) {
    ...
    const int range_end = (d < d_ || has_zero) ?
      neighborhood_size_ + 1 : 1;
...
template <typename TFun>
void LatticeApproximateTraversal::walk_approximate(
    const int start, const int d, const bool has_zero, TFun yield) const {
  if (d <= d_) {
    int walking = start;
...
template <typename T>
void
PermutohedralReverseCpu<T>::init(
  ...
  std::copy(filter, filter + filter_.size(),
            filter_.begin());
...
template<typename T>
void
PermutohedralReverseCpu<T>::compute(const value_type* in, value_type* out) {
...
template<typename T>
void
PermutohedralReverseCpu<T>::max_compute(const value_type* in, value_type* out) {
  std::vector<value_type> blurred_(M_ * num_output_);
...
template <typename T>
void PermutohedralReverseCpu<T>::slice(
...
template <typename T>
void PermutohedralReverseCpu<T>::im2col(
    const value_type* im, const std::size_t value_size,
...
template <typename T>
void PermutohedralReverseCpu<T>::col2im(
    const value_type* col, const std::size_t value_size,
    const std::size_t filter_size, const std::size_t M,
    const std::size_t start, const std::size_t end,
    const std::vector<int>& blur_neighbors,
    value_type* im) {
...
template <typename T>
void PermutohedralReverseCpu<T>::blur(
    const std::vector<value_type>& splatted,
    const value_type* filter, std::vector<value_type>* blurred) const {
  ...
  const std::size_t chunk_size = std::max<std::size_t>(
      1, std::min<std::size_t>(max_size / K, N));
  ...
  for (std::size_t g = 0; g < group_; ++g) {
    for (std::size_t c = 0; c < chunks; ++c) {
      ...
      caffe_cpu_gemm_ex<value_type>(
          ...
          filter + filter_offset * g, K,
          col_data.data(), end - start,
...
template <typename T>
void PermutohedralReverseCpu<T>::max(
    const std::vector<value_type>& splatted,
...
void PermutohedralReverseCpu<T>::splat(
  ...
  for (int i = 0 ; i < in_size_; i++) {
    for (int j = 0; j <= d_; j++) {
      ...
      const value_type& w = lattice_->barycentric_[(in_offset_+i)*(d_+1)+j];
...
template <typename T>
void PermutohedralReverseCpu<T>::reverse(
    const value_type* diff_in, value_type* diff_out_filter,
...
template <typename T>
void PermutohedralReverseCpu<T>::max_reverse(
    const value_type* diff_in, value_type* diff_out_in) {
  std::vector<value_type> sliced_out;
...
void PermutohedralReverseCpu<T>::blur_tick(
    ...
    std::vector<value_type>* blurred_out, value_type* filter_out) {
  ...
  for (std::size_t g = 0; g < group_; ++g) {
    for (std::size_t c = 0; c < chunks; ++c) {
      ...
      caffe_cpu_gemm_ex<value_type>(
          CblasNoTrans, CblasTrans, M, K, end - start,
          static_cast<value_type>(1),
      ...
      // Gradient w.r.t. data.
      caffe_cpu_gemm_ex<value_type>(
          CblasTrans, CblasNoTrans, K, end - start, M,
          static_cast<value_type>(1),
          filter_.data() + filter_offset * g, K,
          blurred_tick.data() + top_offset * g + chunk_size * c, N,
          static_cast<value_type>(0),
          col_diff.data(), end - start);

      col2im(col_diff.data(), value_size_ / group_, size,
             M_, start, end,
    ...
    for (std::size_t j = 0; j < M_; ++j) {
      ...
      for (std::size_t k = 1; k < filter_size; ++k) {
        ...
        if (max_idx_[i * M_ + j] == i * (M_+1) + neighbors[j] + 1){
          (*maxxed_out)[i * (M_+1) + neighbors[j] + 1] += tick_data[i * M_ + j];
        }
      }
    }
  }
...
template <typename T>
void Permutohedral<T>::map_back(
    const std::vector<boost::int16_t>& key, value_type* const x) {
...
void Permutohedral<T>::init(
    const value_type* feature, int data_count, int feature_size,
    int neighborhood_size, bool do_visualization) {
...
template <typename T>
boost::shared_ptr<PermutohedralReverse<T> >
Permutohedral<T>::compute(
    const value_type* filter, const value_type* in,
    int num_output, int group, int value_size, bool do_skip_blur,
    int in_offset, int out_offset, int in_size, int out_size,
  ...
  reverse_operation->init(filter,
                          num_output, group, value_size, do_skip_blur,
...
template <typename T>
boost::shared_ptr<PermutohedralReverse<T> >
Permutohedral<T>::max_compute(const value_type* filter, const value_type* in,
    int value_size,
    int in_offset, int out_offset, int in_size, int out_size,
    value_type* out) {
  ...
  reverse_operation->init(filter,
                          value_size, value_size, value_size, false,
                          in_offset, out_offset, in_size, out_size,
                          lattice_);
